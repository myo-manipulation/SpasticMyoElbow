...
def muscle_spasticity(sim, mus_select, muscle_dynamics, tau, gains, thresholds):
    ...
    if current_time > tau:
        ...
        if muscle_dynamics:
            ...
            excitation_derivative = spastic_excitation_with_dynamics(current_time, current_excitation, current_reflexes, tau, gains)
            spastic_activation = current_excitation + excitation_derivative * 0.02
        else:
            spastic_activation = spastic_excitation_without_dynamics(current_reflexes, gains)
    else:
...
def spastic_excitation_with_dynamics(t, curr_exc, afferent_reflexes, tau, gains):
...
def spastic_excitation_without_dynamics(afferent_reflexes, gains):
...
def plot_data(sim):
    ...
    jointAngle = 180-np.array(sim.jointPos)*180/np.pi
    plot_curve(jointAngle, sim.torques, 'motor torque [Nm]', ax=axes[1])
    ax=axes[1]
    file_path = 'data/ref_data_1.csv'
    refData = pd.read_csv(file_path, header=None)
    refAngle = refData[0]
    refTorque = refData[1]
    ax.plot(refAngle, refTorque, linestyle='-', marker='o')
    ax.legend(['sim', 'ref'])
...
def main():
    ...
    if compensation:
        ...
        calibration_data = np.load('./data/passive_force.npy', allow_pickle=True)
    ...
    refData = load_data()
    ref_names = list(refData.keys())
    
    ref_name = ref_names[8]
    ...
    for i in range(0, 1):
        ...
        gains = [1, 1, 0] # the gain will determine the spasticity type, can be 'ml' for muscle length, 'mv' for muscle velocity, 'mf' for muscle force, 'lv' for multiple feedbacks, 'none' for no spasticity
        No = gains[0]
        thre_factors = [0.35, 0.35, 0]
